...
#Ifdef VT_TTY
    Declare Sub vt_pump_tty()
    Declare Sub vt_internal_tty_shutdown()
    Declare Function vt_internal_tty_init(mode As Long, flags As Long, pages As Long) As Long
    Declare Sub vt_present_tty()
#Endif
#Ifdef VT_USE_SOUND
...
Function vt_internal_ticks() As ULong
    #Ifdef VT_TTY
        Return CULng(Timer * 1000.0)   ' FB built-in, no deps
    #Else
        Return SDL_GetTicks()
    #EndIf
End Function
...
Function vt_init_impl(cols As Long, rows As Long, glyph_w As Long, glyph_h As Long, _
    ...
    ' --- raw TTY guard (SDL2 mode only) ---
    #Ifndef VT_TTY
        #Ifdef __FB_LINUX__
            If Environ("TERM") = "linux" Then
                Print "libvt: raw TTY detected. Recompile with #define VT_TTY."
                End
            End If
        #EndIf
    #EndIf
...
Function vt_key_held(vtscan As Long) As Byte
    ' termios has no live key-state query equivalent.
    If vt_internal.backend = 1 Then Return 0
